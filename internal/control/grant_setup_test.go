...
func TestGuidedSavedGrantRequiresExactFinalConsent(t *testing.T) {
	...
	for _, accept := range []bool{true, false} {
		...
		s := setupSession{control: control, prompt: &setupAnswers{answers: []string{"saved", "yes", "24", "0", "no", "no", "no", final}}, output: io.Discard}
		...
		g := control.saved
		if err != nil || g.Peer.String() != "tgpeer:v1:self:456" || g.Author.String() != "tgpeer:v1:user:456" || g.MinID != 120 || g.MaxID != 120 || g.ReadThrough != 0 || g.Images || g.Documents || g.VoiceNotes || g.Profile != policy.ProfileSelfAuthored || !g.ExpiresAt.Equal(now.Add(24*time.Hour)) {
			t.Fatal(g, err)
		}
	}
}
func TestGuidedChatUsesNumberedImmutableIDAndEscapesTitle(t *testing.T) {
	...
	s := setupSession{control: control, prompt: &setupAnswers{answers: []string{"chats", "yes", "1", "tgpeer:v1:user:456", "consented", "100", "tgmsg:v1:chat:123:120", "2", "120", "yes", "yes", "yes", "yes"}}, output: &out}
	if err := s.guideGrant(context.Background(), time.Now()); err != nil {
		t.Fatal(err)
	}
	if control.saved.Peer != peer || control.saved.MinID != 100 || control.saved.ReadThrough != 120 || !control.saved.Images || !control.saved.Documents || !control.saved.VoiceNotes {
...
func TestGuidedReplacementCanBeDeclined(t *testing.T) {
	...
	s := setupSession{control: c, prompt: &setupAnswers{answers: []string{"saved", "yes", "24", "0", "no", "no", "no", "no"}}, output: io.Discard}
