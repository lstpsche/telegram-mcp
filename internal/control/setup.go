...
func (s *setupSession) chooseAccess(ctx context.Context) error {
	...
	case "full":
		if err := s.confirm(ctx, "Full read exposes all supported conversations, authors, history, images, PDF/plain-text attachments and voice notes to the connected agent and model provider, and permits read acknowledgments. Named scopes do not restrict this authority."); err != nil {
			return err
		}
