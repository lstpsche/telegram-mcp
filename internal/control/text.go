...
func parseGrant(args []string) (policy.Grant, bool) {
	...
	for index := 0; index < len(args); index++ {
		...
		if key == "--allow-images" || key == "--allow-documents" || key == "--allow-voice-notes" {
			permission := &grant.Images
			if key == "--allow-documents" {
				permission = &grant.Documents
			}
			if key == "--allow-voice-notes" {
				permission = &grant.VoiceNotes
			}
			if *permission {
				return policy.Grant{}, false
			}
			*permission = true
			continue
		}
...
func newGrantRecord(grant policy.Grant) grantRecord {
	return grantRecord{Peer: grant.Peer, Author: grant.Author, MinID: grant.MinID, MaxID: grant.MaxID,
		ReadThrough: grant.ReadThrough, Profile: grant.Profile, ExpiresAt: grant.ExpiresAt, Eligible: grant.Eligible, Images: grant.Images, Documents: grant.Documents, VoiceNotes: grant.VoiceNotes}
}
