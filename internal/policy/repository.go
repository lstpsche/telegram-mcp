...
const grantColumns = "peer, author, min_id, max_id, read_through, profile, expires_at, eligible, images, documents, voice_notes"

func scanGrant(row interface{ Scan(...any) error }) (Grant, error) {
	var grant Grant
	var peer, author, expiry string
	if err := row.Scan(&peer, &author, &grant.MinID, &grant.MaxID, &grant.ReadThrough, &grant.Profile, &expiry, &grant.Eligible, &grant.Images, &grant.Documents, &grant.VoiceNotes); err != nil {
		return Grant{}, err
	}
	var err error
...
func (l *Lease) Save(ctx context.Context, grant Grant) error {
	...
	return l.transaction(ctx, func(tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, `INSERT INTO text_grants (peer,authorization_epoch,author,min_id,max_id,read_through,profile,expires_at,eligible,images,documents,voice_notes)
   VALUES (?,?,?,?,?,?,?,?,1,?,?,?)
   ...
   profile=excluded.profile,expires_at=excluded.expires_at,eligible=excluded.eligible,images=excluded.images,documents=excluded.documents,voice_notes=excluded.voice_notes`,
			grant.Peer.String(), l.epoch, grant.Author.String(), grant.MinID, grant.MaxID, grant.ReadThrough, grant.Profile, grant.ExpiresAt.UTC().Format(time.RFC3339Nano), grant.Images, grant.Documents, grant.VoiceNotes)
		if err != nil {
			return internalError(err)
		}
		return nil
...
func (l *Lease) Audit(ctx context.Context, requestID, operation string, category model.ErrorCategory, count int, uncertain bool) error {
	if !model.IsValidRequestID(requestID) || (operation != "list_chats" && operation != "list_messages" && operation != "get_message_context" && operation != "search_messages" && operation != "list_unread" && operation != "list_scopes" && operation != "open_image" && operation != "open_document" && operation != "list_topics" && operation != "open_voice_note" && operation != "catch_up") ||
