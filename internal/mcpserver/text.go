...
func textOutputSchema(kind string) json.RawMessage {
	...
	if kind == "messages" {
		item = `{"type":"object","additionalProperties":false,"required":["id","author","date","text"],"properties":{"id":{"type":"string","pattern":"` + messagePattern + `"},"author":{"type":"string","pattern":"` + peerPattern + `"},"forward":` + forwardSchema + `,"channel_post":` + postSchema + `,"album_id":{"type":"string","pattern":"` + albumPattern + `"},"reply_to":{"type":"string","pattern":"` + messagePattern + `"},"date":{"type":"string"},"reactions":` + reactionsSchema + `,"link_preview":` + linkPreviewSchema + `,"poll":` + pollSchema + `,"reply_chain":{"type":"object","additionalProperties":false,"required":["depth","state"],"properties":{"depth":{"type":"integer","minimum":0,"maximum":5},"state":{"enum":["complete","depth_limit","unavailable"]}}},"text":{"type":"string"},"image":` + imageSchema + `,"document":` + documentSchema + `,"voice_note":` + voiceSchema + `}}`
	}
	if kind == "image" {
	...
	if kind == "search" || kind == "catch_up" {
		item = `{"type":"object","additionalProperties":false,"required":["id","author","date","snippet","snippet_truncated"],"properties":{"id":{"type":"string","pattern":"` + messagePattern + `"},"author":{"type":"string","pattern":"` + peerPattern + `"},"forward":` + forwardSchema + `,"channel_post":` + postSchema + `,"album_id":{"type":"string","pattern":"` + albumPattern + `"},"reply_to":{"type":"string","pattern":"` + messagePattern + `"},"date":{"type":"string"},"reactions":` + reactionsSchema + `,"has_link_preview":{"type":"boolean"},"has_poll":{"type":"boolean"},"snippet":{"type":"string","maxLength":240},"snippet_truncated":{"type":"boolean"},"image":` + imageSchema + `,"document":` + documentSchema + `,"voice_note":` + voiceSchema + `}}`
		next = `{"type":["string","null"],"maxLength":4096}`
	}
	if kind == "unread" {
