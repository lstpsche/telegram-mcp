	...
	for _, tool := range []*mcp.Tool{
		...
		server.AddTool(tool, func(ctx context.Context, request *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if result.Document != nil {
				...
				if model.IsTextDocumentMIME(document.MIMEType) {
...
func callText(ctx context.Context, service *reader.Service, id, name string, args json.RawMessage) (reader.Result, error) {
	...
	case "search_messages":
		return callSearch(ctx, service, id, args)
	case "list_chats":
		input, err := model.DecodeStrict[struct {
			Query  string  `json:"query"`
			Cursor *string `json:"cursor"`
	...
	case "get_message_context":
		...
		if raw, ok := fields["refresh"]; ok {
			var tokens []string
			if len(fields) != 1 || json.Unmarshal(raw, &tokens) != nil || len(tokens) == 0 || len(tokens) > model.MaximumContextTargets {
				return invalid()
			}
			if service == nil {
				return reader.Result{}, model.TextError(model.ErrorNotReady, nil)
			}
			return service.Refresh(ctx, id, tokens)
		}
		input, err := model.DecodeStrict[struct {
...
func textOutputSchema(kind string) json.RawMessage {
	...
	voiceSchema := `{"type":"object","additionalProperties":false,"required":["handle","mime_type","size","duration_seconds"],"properties":{"filename":{"type":"string","maxLength":256,"description":"Untrusted display metadata; never a path or type authority."},"handle":{"type":"string","minLength":1,"maxLength":4096},"mime_type":{"const":"audio/ogg"},"size":{"type":"integer","minimum":1,"maximum":1048576},"duration_seconds":{"type":"integer","minimum":1,"maximum":300}}}`
	documentSchema := `{"type":"object","additionalProperties":false,"required":["handle","mime_type","size"],"properties":{"filename":{"type":"string","maxLength":256,"description":"Untrusted display metadata; never a path or type authority."},"handle":{"type":"string","minLength":1,"maxLength":4096},"mime_type":{"enum":["application/pdf","text/plain","text/markdown","text/csv","application/json"]},"size":{"type":"integer","minimum":1}}}`
	imageSchema := `{"type":"object","additionalProperties":false,"required":["handle","kind","mime_type","width","height","size"],"properties":{"filename":{"type":"string","maxLength":256,"description":"Untrusted display metadata; never a path or type authority."},"handle":{"type":"string","minLength":1,"maxLength":4096},"kind":{"enum":["photo","document"]},"mime_type":{"enum":["image/jpeg","image/png"]},"width":{"type":"integer","minimum":1,"maximum":4096},"height":{"type":"integer","minimum":1,"maximum":4096},"size":{"type":"integer","minimum":1,"maximum":1048576}}}`
	...
	if kind == "search" || kind == "catch_up" {
		item = `{"type":"object","additionalProperties":false,"required":["id","author","date","snippet","snippet_truncated"],"properties":{"edited_at":{"type":"string"},"url":{"type":"string","maxLength":128},"id":{"type":"string","pattern":"` + messagePattern + `"},"author":{"type":"string","pattern":"` + peerPattern + `"},"forward":` + forwardSchema + `,"channel_post":` + postSchema + `,"album_id":{"type":"string","pattern":"` + albumPattern + `"},"reply_to":{"type":"string","pattern":"` + messagePattern + `"},"thread_root":{"type":"string","pattern":"` + messagePattern + `"},"discussion_peer":{"type":"string","pattern":"^tgpeer:v1:channel:[1-9][0-9]*$"},"date":{"type":"string"},"saved_peer":{"type":"string","pattern":"^tgpeer:v1:(self|user|chat|channel):[1-9][0-9]*$"},"pinned":{"type":"boolean"},"reactions":` + reactionsSchema + `,"has_link_preview":{"type":"boolean"},"has_poll":{"type":"boolean"},"snippet":{"type":"string","maxLength":240},"snippet_truncated":{"type":"boolean"},"image":` + imageSchema + `,"document":` + documentSchema + `,"voice_note":` + voiceSchema + `}}`
		next = `{"type":["string","null"],"maxLength":4096}`
	}
	if kind == "unread" {
...
const textEntitiesSchema = `{"type":"array","minItems":1,"maxItems":128,"items":{"type":"object","additionalProperties":false,"required":["kind","offset","length"],"properties":{"kind":{"enum":["unknown","mention","hashtag","bot_command","url","email","bold","italic","code","pre","text_url","mention_name","phone","cashtag","underline","strike","bank_card","spoiler","custom_emoji","blockquote","formatted_date"]},"offset":{"type":"integer","minimum":0,"maximum":65536,"description":"UTF-16 code-unit offset into unchanged text."},"length":{"type":"integer","minimum":1,"maximum":65536,"description":"UTF-16 code-unit length."},"url":{"type":"string","minLength":1,"maxLength":4096},"language":{"type":"string","maxLength":128},"user":{"type":"string","pattern":"^tgpeer:v1:user:[1-9][0-9]*$"},"custom_emoji_id":{"type":"string","pattern":"^-?[1-9][0-9]*$","maxLength":20},"collapsed":{"type":"boolean"},"date":{"type":"integer","minimum":0,"maximum":2147483647},"date_format":{"type":"array","maxItems":6,"uniqueItems":true,"items":{"enum":["relative","short_time","long_time","short_date","long_date","day_of_week"]}}}}}`
