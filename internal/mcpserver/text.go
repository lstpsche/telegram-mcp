...
func registerTextTools(server *mcp.Server, service *reader.Service) {
	...
	for _, tool := range []*mcp.Tool{
		{Name: "list_chats", Description: "List up to 20 currently granted conversations. Fetches only granted peer metadata and does not acknowledge history.", InputSchema: json.RawMessage(`{"type":"object","additionalProperties":false,"properties":{"limit":{"type":"integer","minimum":1,"maximum":100}}}`), OutputSchema: textOutputSchema(false), Annotations: &mcp.ToolAnnotations{ReadOnlyHint: true, IdempotentHint: true, OpenWorldHint: &open}},
		{Name: "list_messages", Description: "Read bounded authorized text history, newest first. Before is an exclusive message reference, never authority. Marks the separately authorized dialog prefix read before releasing bodies. next_cursor is null; use a returned ID as before for an older window.", InputSchema: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["peer"],"properties":{"peer":{"type":"string","pattern":"` + peerPattern + `"},"before":{"type":"string","pattern":"` + messagePattern + `"},"limit":{"type":"integer","minimum":1,"maximum":100}}}`), OutputSchema: textOutputSchema(true), Annotations: &mcp.ToolAnnotations{ReadOnlyHint: false, IdempotentHint: false, OpenWorldHint: &open}},
		{Name: "get_message_context", Description: "Read one authorized target and bounded older/newer neighbors. Zero neighbors reads just the target. Missing or denied targets yield no bodies. Marks the authorized dialog prefix read before releasing text.", InputSchema: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["message"],"properties":{"message":{"type":"string","pattern":"` + messagePattern + `"},"before":{"type":"integer","minimum":0,"maximum":49},"after":{"type":"integer","minimum":0,"maximum":49}}}`), OutputSchema: textOutputSchema(true), Annotations: &mcp.ToolAnnotations{ReadOnlyHint: false, IdempotentHint: false, OpenWorldHint: &open}},
	} {
...
func textOutputSchema(messages bool) json.RawMessage {
	...
	return json.RawMessage(strings.ReplaceAll(`{"type":"object","additionalProperties":false,"required":["schema_version","request_id","freshness","partial","read_effect","items","next_cursor","warnings","untrusted_content"],"properties":{"schema_version":{"const":"1"},"request_id":{"type":"string"},"freshness":{"type":"object","additionalProperties":false,"required":["telegram","checked_at"],"properties":{"telegram":{"enum":["live","recovering","stale","partial","unavailable"]},"checked_at":{"type":"string"}}},"partial":{"type":"boolean"},"read_effect":{"type":"object","additionalProperties":false,"required":["kind"],"properties":{"kind":{"enum":["none","history_marked_read"]},"through_message_id":{"type":"string"}}},"items":{"type":"array","maxItems":100,"items":ITEM_SCHEMA},"next_cursor":{"type":"null"},"warnings":{"type":"array","items":{"enum":["partial_result","freshness_degraded"]}},"untrusted_content":{"const":true}}}`, "ITEM_SCHEMA", item))
}
