...
func callText(ctx context.Context, service *reader.Service, id, name string, args json.RawMessage) (reader.Result, error) {
	...
	case "get_message_context":
		...
		for _, value := range values {
			query, err := reader.ContextReference(value, model.HistoryQuery{BeforeCount: input.Before, AfterCount: input.After, ReplyDepth: input.ReplyDepth, ResolveDiscussion: input.ResolveDiscussion, Limit: input.Before + input.After + 1})
			if err != nil {
				return invalid()
			}
