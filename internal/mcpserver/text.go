...
func registerTextTools(server *mcp.Server, service *reader.Service) {
	...
	for _, tool := range []*mcp.Tool{
		...
		{Name: "get_message_context", Description: "Read one authorized target and bounded older/newer neighbors. With zero neighbors and reply_depth, reads just the target. With resolve_discussion true, Full read can resolve a channel post to discussion_root in its joined non-forum discussion group. This returns a reference, not group bodies or receipts; use that group as peer and the root as thread_root in search_messages. Resolution failure withholds the entire result. Optional reply_depth (0–5, default 0) adds authorized same-conversation parents; reply_chain on the target reports complete, depth_limit or unavailable. The target, neighbor and reply-depth bounds must sum to at most 100. Missing, excluded and denied parents share unavailable. Missing or denied targets yield no bodies. Marks the authorized dialog prefix read before releasing text and permitted image, document and voice-note metadata.", InputSchema: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["message"],"properties":{"message":{"type":"string","pattern":"` + messagePattern + `"},"before":{"type":"integer","minimum":0,"maximum":49},"after":{"type":"integer","minimum":0,"maximum":49},"resolve_discussion":{"type":"boolean"},"reply_depth":{"type":"integer","minimum":0,"maximum":5}}}`), OutputSchema: textOutputSchema("messages"), Annotations: &mcp.ToolAnnotations{ReadOnlyHint: false, IdempotentHint: false, OpenWorldHint: &open}},
...
func callText(ctx context.Context, service *reader.Service, id, name string, args json.RawMessage) (reader.Result, error) {
	...
	case "get_message_context":
		...
		query.ResolveDiscussion = input.ResolveDiscussion
...
func textOutputSchema(kind string) json.RawMessage {
	...
	if kind == "search" || kind == "catch_up" {
		item = `{"type":"object","additionalProperties":false,"required":["id","author","date","snippet","snippet_truncated"],"properties":{"id":{"type":"string","pattern":"` + messagePattern + `"},"author":{"type":"string","pattern":"` + peerPattern + `"},"forward":` + forwardSchema + `,"channel_post":` + postSchema + `,"album_id":{"type":"string","pattern":"` + albumPattern + `"},"reply_to":{"type":"string","pattern":"` + messagePattern + `"},"thread_root":{"type":"string","pattern":"` + messagePattern + `"},"discussion_peer":{"type":"string","pattern":"^tgpeer:v1:channel:[1-9][0-9]*$"},"date":{"type":"string"},"saved_peer":{"type":"string","pattern":"^tgpeer:v1:(self|user|chat|channel):[1-9][0-9]*$"},"pinned":{"type":"boolean"},"reactions":` + reactionsSchema + `,"has_link_preview":{"type":"boolean"},"has_poll":{"type":"boolean"},"snippet":{"type":"string","maxLength":240},"snippet_truncated":{"type":"boolean"},"image":` + imageSchema + `,"document":` + documentSchema + `,"voice_note":` + voiceSchema + `}}`
		next = `{"type":["string","null"],"maxLength":4096}`
	}
	if kind == "unread" {
