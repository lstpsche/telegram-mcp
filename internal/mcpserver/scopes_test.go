...
func TestAgentDiscoversScopeSearchesTwoPeersAndOpensExactContext(t *testing.T) {
	...
	first := call("search_messages", map[string]any{"scope": discovered["id"], "query": "synthetic", "limit": 1})
	next := call("search_messages", map[string]any{"scope": discovered["id"], "query": "synthetic", "limit": 1, "cursor": first["next_cursor"]})
