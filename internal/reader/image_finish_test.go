...
func TestOpenMediaRejectsAuthorityLossDuringAudit(t *testing.T) {
	for _, document := range []bool{false, true} {
		for _, loss := range []string{"expiry", "cancel", "readiness"} {
			t.Run(loss, func(t *testing.T) {
				var s *Service
				var backend *imageFake
				var handle string
				if document {
					s, backend, _, _, handle = documentService(t, "text/plain")
				} else {
				...
				if model.TextErrorCategory(err) != model.ErrorReadEffectUncertain || result.Image != nil || result.Document != nil || len(result.JSON) != 0 {
